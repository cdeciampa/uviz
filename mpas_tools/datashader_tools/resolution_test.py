...
from plotting.utils import nonlinear_colorbar
...
def datashader_wrapper(mesh_ds, unstructured_ds, primalVarName, time, level=None, 
                       ...
                       y_sampling=None, x_range=None, y_range=None):
...
target_var = 'U10'
target_time = 17
pixel_ratio = 10 # 15 is still too pixelated but >= 17 crashes bokeh
lat_bounds = (2.418E6, 3.335E6)
lon_bounds = (-8.278E6, -6.459E6)
title = f"0831 TS {target_time} {target_var} PR {pixel_ratio}"
print('Rasterizing data.')
rasterized_0831 = datashader_wrapper(hires_mesh, hires_0831_ds, target_var, target_time, pixel_ratio=pixel_ratio, x_range=lon_bounds, y_range=lat_bounds)
final = rasterized_0831.opts(tools=['hover'], colorbar=True, cmap='jet',
                             title=title) * gf.coastline(projection=projection).options(scale='10m') # this works
print('Saving output image.')
hv.save(final, f'../datashader/output_imgs/{title}.png', backend='bokeh', dpi=300)
end = time.time()
print(f'Time elapsed: {end} seconds for {pixel_ratio} PR.')
